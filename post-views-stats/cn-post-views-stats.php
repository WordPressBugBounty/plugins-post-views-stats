...
function cn_tpv_db_install() {
	...
	// Securely check if table exists
	...
	if ( $table_exists != $table_name ) {
		...
		// Safe and compliant SQL structure
...
function track_post_view() {
	...
	// Skip revisions and previews
	if ( wp_is_post_revision( $post ) || is_preview() ) {
		return;
	}

	// Only track for single posts or pages
	if ( is_single() || is_page() ) {
		...
		if ( ! cn_tpv_isBot( $ua ) ) {
			...
			$created_at  = time();
			$create_date = current_time( 'Y-m-d' ); // safer than date('Y-m-d')

			// Secure insert query using $wpdb->prepare()
			$query = $wpdb->prepare(
				"INSERT INTO $table_name ( post_id, created_at, create_date )
				 VALUES ( %d, %d, %s )",
				$post_id,
				$created_at,
				$create_date
			...
			// if ( $results ) file_put_contents( dirname( __FILE__ ) . '/log.txt', $post_id . ', ', FILE_APPEND );
		}
	}
}
...
function cn_tpv_get_view_count( $post_ID ) {
	global $wpdb;
	$table_name = $wpdb->prefix . "cn_track_post";
	// Sanitize and ensure $post_ID is an integer
	$post_ID = intval( $post_ID );
	// Use wpdb::prepare() to safely include variable
	$query = $wpdb->prepare(
	...
	$view_count = isset( $tabledata->counts ) ? intval( $tabledata->counts ) : 0;
	return $view_count;
}
